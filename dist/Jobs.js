...
class Jobs {
    constructor() {
        this.LinksJobs = Quests_1.Quests;
        this.adjustRatio();
    ...
    findBestSchedule() {
        ...
        const jobsWithMaxYield = [];
        const availableQuests = this.LinksJobs;
        const maxUnemployedDays = 2;
        const latestPossibleStartDate = availableQuests.sort((a, b) => -1 * (a.startDate - b.startDate))[0].startDate;
        while (nextJobStartDate <= latestPossibleStartDate) {
            // list of next possible jobs
            const nextPossibleJobs = [
        ...
        for (let index = 0; index < this.LinksJobs.length - 1; index++) {
            ...
            if (thisQuest && nextQuest) {
                ...
                if (thisQuest.duration !== nextQuest.startDate - thisQuest.startDate) {
                    const durationBetweenStartOfTwoJobs = nextQuest.startDate - thisQuest.startDate;
                    // only return if reward is higher, has a same start date and finishes before next quest
                    const betterQuest = Quests_1.Quests.find(quest => quest.startDate === thisQuest.startDate &&
                    ...
                    // try to find a quest between jobs
                        ...
                        const thisQuestFinishDate = thisQuest.startDate + thisQuest.duration;
                        ...
                        if (nextQuest.startDate !== thisQuestFinishDate) {
                            const QuestInBetween = Quests_1.Quests.find(quest => quest.startDate === thisQuestFinishDate &&
